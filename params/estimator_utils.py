...
def create_estimator(master,
    ...
    """
    This function is called in train.py it create estimator that handle load data, handle exceptions,
    create checkpoint and train your model
    Args:
        master(str): Master tensorflow server (empty string for local)
        model_dir(str): Output directory for model checkpoints
        use_tpu(Bool): Whether to run on TPU accelerators
        iterations_per_loop(int): Number of iterations to perform per TPU loop
        num_shards(int): Number of TPU shards available
        model_params(List): list of parameters of model
        include_features_in_predictions(Bool):
        decode_keys(tuple):
        train_init_checkpoint:
        train_warmup_steps:
        save_checkpoints_steps:
        keep_checkpoint_max(Integer): The maximum number of recent checkpoint files to keep.
                                        As new files are created, older files are deleted.
    Returns:
        tensorflow estimator(instance of class): can be trained ........
    """
...
def _estimator_model_fn(use_tpu, model_params, model_dir,
  ...
  def model_fn(features, labels, mode, config, params):
    ...
    features: A tf.Tensor or a dictionary of string feature name to Tensor.
                features are consumed by model_fn. They should satisfy the expectation of model_fn from inputs.
    """
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:#check if mode == predict in model
      predictions = model_params.estimator_prediction_fn(features) # pass inputs to prediction function
